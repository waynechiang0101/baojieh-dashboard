...
**任何需要人工介入的步驟都是錯的，必須找到自動化方法。**

- ❌ 「請助理把 XLS 放到 Downloads」→ 錯，改從系統直接抓
- ❌ 「每月手動上傳 iWMS 檔案」→ 暫時妥協，長期目標是直接呼叫 iWMS API
- ❌ 「點 bookmarklet 更新 token」→ 錯，改用自動登入換 token
- ✅ DERP 每日 18:30 GitHub Actions 自動抓，不需要任何人工

**遇到「沒辦法自動化」時，先問：**
1. 有沒有 API？
2. 能不能爬網頁？
3. 能不能從其他系統間接取得？
4. 實在不行，才接受暫時人工，但要記錄為 TODO。
...
- P&G 本月業績（全通路）= DERP `total_mo`（含康是美/CVS直送/盤商）
- 業務通路本月 = `total_mo - km_derp - cvs_others - pander_derp`（排除直送+盤商）
- 藥房+超市業務 ≈ 業務通路本月（差 XLS G27+G28 約 2-3%，正常口徑差）
- 藥房 AC通路 = 大小型藥局 + 月子中心 + 大小型嬰兒專門店
- 超市 AC通路 = 超級市場 + 大小型小超（不含盤商）
- 盤商 = 盤商:量販/超市/傳統商店 + 盤商:專業領域通路(國外)
- 美妝店($50M) = DERP 把康是美歸類為美妝店，實際是 km_derp
- IYA：`iy5` = 季 vs 去年季，`iy3` = 本月 vs 去年本月
- 劉暄芸 `tgt=0` 正常（管康是美直送，沒有月目標）
- 月趨勢圖 12-3 月為估算值
- 季累計 DERP timeout → 自動用本月資料頂替，看板繼續更新
...
## 庫存健康預警模組（2026-06-10 新增）

- `parse_inventory_health(path)`：重用每日下載的 327-50 庫存檔，零額外請求
- 燈號（主力倉=主貨倉/出貨倉/康是美倉，依庫存天數）：60+🟡 / 90+🟠 / 120+🔴D級 / 180+⚫；**無動銷（日均銷=0）與下架品有庫存直接進D級**
- 即期倉/壞品倉獨立統計（is_expiry='即期' in 倉名, is_damaged='壞品'）
- 出清損失假設（顯示在頁面，推動決策用）：現在出清虧8~20%、拖到180天虧70%、提早處理省50%
- 寫入 `const INV_HEALTH={...}`，每日自動更新；頁面 `pg-inv-health`
...
### DC 庫存（2026-06-11 新增）
- 平台選單「銷售庫存→庫存數量查詢」：物流庫存=捷盟北倉+南倉+西園倉（康是美三個DC）
- 欄位：品號/條碼/品名/三倉休配狀態/**訂單在途量**/物流庫存/三倉明細/庫存日期
- `fetch_km_inventory(page)` 每週一隨實銷一起抓，by_sku 附 `dc`/`transit`/`kst`（休配狀態，4=停售下架）
- 看板：品牌表加「DC庫存/可賣週數」（DC÷週均實銷，>6週紅/>4週橘），SKU明細同+「康停售」badge
- **可賣週數 = 真消化率指標**（同單位、無網購干擾），取代 SU 那條死路
