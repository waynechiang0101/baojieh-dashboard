...
def main():
    ...
    print("\n[LINE 推播]")
    push_line_summary(total_mo, total_iya_mo, iya_mo_pct, ar_unpaid, inv_health)


def push_line_summary(total_mo, total_iya_mo, iya_mo_pct, ar_unpaid, inv_health):
    """每日看板更新後推播關鍵燈號到 LINE。
    需要環境變數: LINE_CHANNEL_TOKEN, LINE_USER_ID
    """
    import json as _json
    ...
    if not token or not user_id:
        print("  ⚠ 未設定 LINE_CHANNEL_TOKEN / LINE_USER_ID，跳過推播")
        return
    ...
    iya_sign = "▲" if iya_mo_pct >= 0 else "▼"
    lines = [
        f"📊 寶捷業績 {today}",
        f"本月: {fm_m(total_mo)} ({iya_sign}{abs(iya_mo_pct):.1f}% IYA)",
        f"應收未付: {fm_m(ar_unpaid)}" if ar_unpaid else "",
    ]
    ...
    if inv_health:
        ...
        lines.append("庫存預警")
        if lt['black']['n']:
            lines.append(f"⚫ D級 {lt['black']['n']} SKU / {fm_m(lt['black']['amt'])}")
        if lt['red']['n']:
            lines.append(f"🔴 紅燈 {lt['red']['n']} SKU / {fm_m(lt['red']['amt'])}")
        if lt['orange']['n']:
            lines.append(f"🟠 橘燈 {lt['orange']['n']} SKU / {fm_m(lt['orange']['amt'])}")
        exp = inv_health.get('expiry_total', 0)
        dmg = inv_health.get('damaged_total', 0)
        if exp or dmg:
            lines.append(f"即期 {fm_m(exp)} | 壞品 {fm_m(dmg)}")
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            print(f"  ✓ LINE 推播成功 (status={resp.status})")
    except Exception as e:
        print(f"  ⚠ LINE 推播失敗: {e}")
